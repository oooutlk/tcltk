...
impl Parse for TkbindInput {
    fn parse( input: ParseStream ) -> parse::Result<Self> {
        ...
        Ok( TkbindInput{ tk, cmd, args, bind, closure })
    }
}
...
/// # Syntax
///
/// `tkbind!( $interp:expr, cmd:$cmd:expr, args:$args:expr, bind:($($bind: bind_syn::Bind),*), $closure:expr )`
///
/// # Input parameters
///
...
pub fn tkbind( input: TokenStream ) -> TokenStream {
    let TkbindInput{ tk, cmd, args, bind, mut closure } = parse_macro_input!( input as TkbindInput );
    ...
    let expanded = quote!{{
        ...
        let address_as_name = format!( "__tkbind_closure_{:?}", #random_value.wrapping_add( client_data as usize ));
