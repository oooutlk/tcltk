...
impl TkKey {
    pub fn as_char( &self ) -> char {
        i32::from( *self ) as u8 as char
    }
}

impl TryFrom<Obj> for TkKey {
    type Error = TkInvalidKeySym;

    fn try_from( obj: Obj ) -> Result<TkKey, Self::Error> {
        if let Ok(i) = i32::try_from( obj.clone() ) {
            if let Ok( key ) = TkKey::try_from(i) {
                return Ok( key );
            }
        }
        Err( TkInvalidKeySym( obj ))
    }
}
